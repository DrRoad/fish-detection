...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels):
    ...
    for c in range(1, n_classes):
        ...
        for d in range(n_class_detections):
            ...
            # 'ind' the index of the obj in the image-level tensors 'object_boxes' (n_class_objects_in_img)
            # 'original_ind' the original class-level tensors 'true_class_boxes' (n_class_objects)
            original_ind = torch.LongTensor(range(n_class_objects))[true_class_images == this_image][ind]
...
def accuracy_topk(scores, targets, k):
    """
